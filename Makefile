...
.PHONY:	all clean lint tests test test-tls test-multi-secret test-secret-limit test-ip-acl test-drs-delays test-unix-socket docker-image-amd64 docker-run-help-amd64 docker-image-arm64 docker-run-help-arm64 fuzz fuzz-run
...
test-unix-socket:
	@echo "Building and running Unix socket unit tests in Docker..."
	@docker buildx build --platform linux/amd64 --load -t teleproxy:test-unix-build . --target builder -q
	@docker run --rm --platform linux/amd64 teleproxy:test-unix-build sh -c \
		"cd /src && gcc -std=gnu11 -O2 -Wall -o /tmp/test_unix_socket tests/test_unix_socket.c -lpthread && /tmp/test_unix_socket"
